...
class cmdvel2gazebo:
    ...
    def __init__(self,ns):
        ...
        # The initial velocity and tire angle are 0
        self.x = 0
        self.z = 0
        ...
        # Setting number of seconds delay for the Dead Man's Switch
    ...
    def publish(self):
        ...
        # Note that the tire angle will not change.
        # Also, we only set self.x to be 0 after 200ms of timeout
        ...
        if self.z != 0:
            ...
            # if we aren't turning, the process is easier
            msgRear = Float64()
            msgRear.data = self.x;
            self.pub_rearL.publish(msgRear)
            self.pub_rearR.publish(msgRear)
